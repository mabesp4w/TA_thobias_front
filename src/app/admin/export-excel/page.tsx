...
interface ExportForm {
  ...
  umkmId?: string; // Tambahkan field untuk UMKM ID
}
const ExportExcelPage = () => {
  const { setWelcome } = useWelcomeContext();
  const [isLoading, setIsLoading] = useState(false);
  const [downloadHistory, setDownloadHistory] = useState<any[]>([]);
  const [umkmList, setUmkmList] = useState<any[]>([]); // State untuk list UMKM
  ...
  const {
    ...
    defaultValues: {
      ...
      umkmId: "", // Default kosong berarti semua
    },
  });
  ...
  // Function untuk fetch list UMKM dari API
  const fetchUmkmList = async () => {
    try {
      const token = await useLogin.getState().setToken();
      const response = await fetch(`${BASE_URL}/api/profil-umkm/`, {
        // Asumsi endpoint baru untuk list UMKM
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      if (!response.ok) throw new Error("Gagal fetch list UMKM");
      const data = await response.json();
      console.log({ data });
      setUmkmList(data); // Asumsi data adalah array [{id, name}, ...]
    } catch (error) {
      console.error("Error fetching UMKM:", error);
      toast.error("Gagal memuat list UMKM");
    }
  };
  ...
  const onSubmit = async (data: ExportForm) => {
    ...
    try {
      ...
      // Tambahkan umkm_id jika dipilih
      if (data.umkmId) {
        params.append("umkm_id", data.umkmId);
        const umkm = umkmList.find((umkm) => umkm.id === data.umkmId);
        umkmNm = umkm?.nm_bisnis || "";
      }

      if (data.exportType === "sales_report") {
        url = `${BASE_URL}/api/export-excel/export_sales_report/?${params.toString()}`;
        filename = `laporan_penjualan_${umkmNm}_${moment().format(
          "YYYYMMDD_HHmmss"
        )}.xlsx`;
      } else {
        url = `${BASE_URL}/api/export-excel/export_sales_analysis/?${params.toString()}`; // Tambahkan params juga ke analysis
        filename = `analisis_penjualan_${umkmNm}_${moment().format(
          "YYYYMMDD_HHmmss"
        )}.xlsx`;
      }
                      ...
                      >
                        ...
                        {umkmList.map((umkm) => (
                          <option key={umkm.id} value={umkm.id}>
                            {umkm.nm_bisnis} {/* Asumsi field name di data */}
                          </option>
                        ))}
                      </select>
                    )}
