...
const Form = ({ dtEdit, halaman }: Props) => {
  // store
  const { addData, updateData } = useProvinsi();
  // state
  ...
  const {
    ...
    formState: { errors },
    watch,
  } = useForm<ProvinsiType>();

  // reset form
  const resetForm = () => {
    setValue("id", "");
    setValue("nm_provinsi", "");
  };
  // data edit
  useEffect(() => {
    if (dtEdit) {
      setValue("id", dtEdit.id);
      setValue("nm_provinsi", dtEdit.nm_provinsi);
    } else {
      resetForm();
    }
  ...
  // simpan data
  const onSubmit: SubmitHandler<ProvinsiType> = async (row) => {
    //  submit data
    // console.log({ row });
    // return;
    submitData({
  ...
  return (
    <ModalDef id="add_provinsi" title={`Form ${halaman}`}>
