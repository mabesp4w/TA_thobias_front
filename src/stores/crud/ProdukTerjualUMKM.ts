...
const useProdukTerjualUMKM = create(
  devtools<Store>((set) => ({
    dtProdukTerjual: {
      last_page: 0,
      current_page: 0,
    ...
    addData: async (row) => {
      ...
      try {
        const res = await crud({
          method: "post",
          url: `/produk-terjual/create_my_sale/`,
          headers: { Authorization: `Bearer ${token}` },
          data: row,
        });
        set((prevState) => ({
    ...
    removeData: async (id) => {
      ...
      try {
        const res = await crud({
          method: "delete",
          url: `/produk-terjual/${id}/destroy_my_sale/`,
          headers: { Authorization: `Bearer ${token}` },
        });
        set((prevState) => ({
          dtProdukTerjual: {
    ...
    updateData: async (id, row) => {
      ...
      try {
        const response = await crud({
          method: "PUT",
          url: `/produk-terjual/${id}/update_my_sale/`,
          headers: { Authorization: `Bearer ${token}` },
          data: row,
        });
        set((prevState) => ({
...
export default useProdukTerjualUMKM;
